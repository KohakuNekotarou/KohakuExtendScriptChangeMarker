...
public:
	...
	// 表示だけ切り替える(ミドル・シングルクリックで反転)。kFalse の間はこれら全部を描かない。旧版べた載せ
	// (sShowOriginal)はこのトグルの影響を受けない(ダブルクリックで別管理)。
...
bool16 KESCMDrawEventHandler::sMarksVisible = kTrue;	// 既定=表示(上書き表示の master トグル)
...
// ミドルボタンの「ダブルクリック＆ホールド」判定。シングルクリックでは覗かない(誤発火防止)。
// 2回目の押下が前回押下から kKESCMPeekDblInterval 秒以内なら「ダブルクリックの2打目」とみなし、
// その押下を保持している間だけ旧版を表示する(離すと消す)。IEvent::GetTime() は起動からの秒。
static const PMReal kKESCMPeekDblInterval = 0.5;	// ダブルと判定する2打の最大間隔(秒)
static PMReal sPeekLastMDownTime = -1.0;	// 直前のミドル押下時刻(秒)。-1=無し
static bool16 sPeekActive        = kFalse;	// ダブルの2打目を押し込み中(=覗き表示中)か
static bool16 sSingleHiding      = kFalse;	// シングルのミドル押下中(=全マークを一時的に非表示中)か。離すと戻す
...
enum KESCMPeekResult { kKESCMPeekNoView = 0, kKESCMPeekNoSpread = 1, kKESCMPeekShown = 2 };
...
// ダブルの2打目を検出したときの共通処理: 「保持中だけ覗く」状態に入り、マウス下スプレッドの旧版を表示。
// 連鎖(3連打)防止に押下時刻はリセットする。覗き中もハンドツールにして「旧状態で掴んで移動」できるように。
// 覗きに入る時はシングルの一時非表示を解除しマークを通常(表示)へ戻す(覗き中は旧版が覆うので見た目に
// 影響せず、離した後はマークが正常表示に戻る)。
static void KESCMBeginPeekHold()
{
	sPeekActive = kTrue;
	sPeekLastMDownTime = -1.0;
	sSingleHiding = kFalse;
	KESCMDrawEventHandler::sMarksVisible = kTrue;
...
//========================================================================================
// KESCMPeekWatcher
//   非消費のイベントウォッチャ。peek が arm 済み(kescmArmMousePeek)の間、ミドルボタンを押すと
//   マウス下スプレッドの旧版べた載せを表示し、離すと隠す。非消費=ミドルボタン本来の動作も走る。
//========================================================================================
...
{
	// 興味=ミドル押下/解放＋ダブルクリック。毎回返す(空を返すと監視解除される)。
	// ダブルクリックはネイティブの kDoubleClick で受ける(Windows では2打目は WM_MBUTTONDBLCLK=
	// kDoubleClick として来て、2回目の kMButtonDn は来ないため、自前タイマだけでは取りこぼす)。
	IEventDispatcher::EventTypeList interest(IEvent::kMButtonDn, IEvent::kMButtonUp, IEvent::kDoubleClick);

	if (!sPeekArmed || e == nil)
		return interest;

	const IEvent::EventType type = e->GetType();
	if (type != IEvent::kMButtonDn && type != IEvent::kMButtonUp && type != IEvent::kDoubleClick)
		return interest;
	    ...
	    docList->FindDocByDataBase(sPeekSourceDB) == nil)
	{
		...
		sSingleHiding = kFalse;
		sPeekLastMDownTime = -1.0;
		KESCMDrawEventHandler::sMarksVisible = kTrue;	// シングルで隠したままにしない
		KESCMDrawEventHandler::DropAllOrig();
		return interest;
	}

	const PMReal now = e->GetTime();

	if (type == IEvent::kDoubleClick)
	{
		// ネイティブのダブルクリック(OS のダブルクリック時間を尊重)。IEvent はどのボタンかを持たないので、
		// 「直前のミドル押下が間隔以内」なら=ミドルのダブルクリックとみなす。保持中だけ旧版を表示。
		// Windows ではミドルの2打目はこの kDoubleClick として来る(2回目の kMButtonDn は来ない)。
		if (sPeekLastMDownTime >= 0 && (now - sPeekLastMDownTime) <= kKESCMPeekDblInterval)
			KESCMBeginPeekHold();
	}
	else if (type == IEvent::kMButtonDn)
	{
		// フォールバック(kDoubleClick を出さない環境): 2打目が間隔以内なら自前検出でダブル=覗きへ。
		// 通常(Windows)はここは常に1打目になり、2打目は上の kDoubleClick で処理される。
		if (sPeekLastMDownTime >= 0 && (now - sPeekLastMDownTime) <= kKESCMPeekDblInterval)
		{
			KESCMBeginPeekHold();
		}
		...
		{
			// シングル動作(押している間だけ): 全マーク(リング＋X＋変更数＋オーバーセット)を一時的に隠し、
			// ハンドツールに切替えて「きれいな状態で掴んで移動」できるようにする。
			// 離す(kMButtonUp)とマークもツールも元に戻す。Windows ではダブルの1打目もここを通るが、直後の Up
			// で戻り、続く kDoubleClick で覗きへ移る(一瞬の非表示は再描画が遅延されるため通常ちらつかない)。
			sPeekLastMDownTime = now;
			sSingleHiding = kTrue;
			KESCMDrawEventHandler::sMarksVisible = kFalse;
			KESCMEnterHandTool();	// きれいな状態で掴んで移動
	...
	else // kMButtonUp
	{
		...
		if (sPeekActive)
		{
			// ダブルの2打目を離した → 旧版を隠す(マークは触らない)。キャッシュは保持(再 peek は即時)。
			sPeekActive = kFalse;
		...
		else if (sSingleHiding)
		{
			// シングルの押下を離した → 一時的に隠していた全マークを元の表示に戻す。
			sSingleHiding = kFalse;
			KESCMDrawEventHandler::sMarksVisible = kTrue;
			InterfacePtr<IDocument> doc(sPeekTargetDB, sPeekTargetDB->GetRootUID(), UseDefaultIID());
			if (doc != nil)
...
void KESCMPeekWatcher::StartWatching()
{
	...
	if (dispatcher)
	{
		dispatcher->AddEventWatcher(this, IEventDispatcher::EventTypeList(IEvent::kMButtonDn, IEvent::kMButtonUp, IEvent::kDoubleClick));
		fWatching = kTrue;
	}
}
...
ErrorCode KESCMScriptProvider::ArmMousePeek(ScriptID methodID, IScriptRequestData* data, IScript* parent)
{
	...
	sPeekArmed = kTrue;
	sPeekActive = kFalse;			// ダブルクリック判定状態を初期化
	sSingleHiding = kFalse;
	sPeekLastMDownTime = -1.0;

	PMString report("kescm: mouse peek armed (double-click+hold the middle button over a spread)");
	report.SetTranslatable(kFalse);
	ScriptData rd; rd.SetPMString(report);
	data->AppendReturnData(parent, methodID, rd);
	return kSuccess;
